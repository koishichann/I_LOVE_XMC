...
from ..utils.utils import load_map, read_index, read_label_text, read_text, transfer_indexs_to_labels, \
    transfer_labels_to_index
from ..utils import utils

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def rank_train(args, type):
    '''
    src_text: train set的documents
    pred_dir: train pred/combine labels(text), not index
    ground_truth_label:
    '''
    data_dir = utils.BASE_DATASET_DIR
    model_name = args.rank_model
    src_text = os.path.join(utils.BASE_DATASET_DIR, 'X.trn.txt')
    model_save_dir = os.path.join(utils.RANK_RECORDS_DIR, 'res', utils.RANK_MODEL_NAME, 'model_save')
    batch_size = args.rank_batch
    epochs = args.rank_epoch
    pred_dir = os.path.join(utils.COMBINE_RECORDS_DIR, 'res', utils.COMBINE_MODEL_NAME,
                            type + '_combine.txt')
    ...
    with open(model_save_dir + "build_labels.txt", "w+", encoding='utf-8', errors='ignore') as fb:
...
def label_rank(args, type) -> \
    ...
    '''
    ###
    src_labels = os.path.join(utils.COMBINE_RECORDS_DIR, 'res', utils.COMBINE_MODEL_NAME,
                              type + '_combine.txt')
    output_text = utils.OUTPUT_TST_TEXT_DIR
    output_index = utils.OUTPUT_TST_INDEX_DIR

    src_text = os.path.join(utils.BASE_DATASET_DIR, 'X.tst.txt')
    model_type = args.rank_type
    model_name = args.rank_model
    label_map = os.path.join(utils.BASE_DATASET_DIR, 'output-items.txt')

    if args.is_rank_train:
        ###
        model_save_dir = os.path.join(utils.RANK_RECORDS_DIR, 'res', utils.RANK_MODEL_NAME, 'model_save')
        rank_model = SentenceTransformer.load(model_save_dir)
    else:
        if model_type in 'bi':
    ...
    if output_text:
        with open(output_text, 'w+', encoding='utf-8', errors='ignore') as w1:
            for row in ranked_list:
                w1.write(",".join(row) + '\n')
        with open(output_text.rstrip(".txt") + "_score.txt", 'w+', encoding='utf-8', errors='ignore') as w2:
    ...
    if label_map:
        ...
        if output_index:
            with open(output_index, 'w+', encoding='utf-8', errors='ignore') as w1:
                for row in ranked_list:
